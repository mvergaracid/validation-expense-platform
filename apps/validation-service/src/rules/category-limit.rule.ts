  ...
  evaluate(context: ValidationContext): void {
    ...
    if (!limites) {
      context.addSuggestion(
        ...
        `No existe configuración de límites para la categoría ${categoria}, se aprueba por omisión.`,
      );
      return;
    }
    ...
    if (monto <= limites.pendiente_hasta) {
      context.addSuggestion(
        ...
        `Requiere revisión: el monto ${monto} ${monedaBase} supera el umbral automático (${limites.aprobado_hasta}) pero permanece por debajo del máximo (${limites.pendiente_hasta}).`,
      );
      return;
    }
    ...
    context.addSuggestion(
      ...
      `El monto ${monto} ${monedaBase} excede el límite máximo (${limites.pendiente_hasta}).`,
    );
  }
}
